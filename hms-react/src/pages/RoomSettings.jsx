...
export default function RoomSettings() {
  ...
  const roomTypes = ['General Ward', 'Semi-Private', 'Private Room', 'ICU'];
  ...
  const fetchConfigs = async () => {
    ...
    try {
      ...
      const merged = roomTypes.map(roomType => {
        const existing = data.find(c => c.roomType === roomType);
        if (existing) {
          return existing;
        }
        return { roomType, ...defaults[roomType] };
      });
      setRoomConfigs(merged);
    } catch (err) {
      console.error('Failed to load room settings', err);
      // Load defaults on error
      const defaults = roomTypes.map(roomType => ({
        roomType,
        dailyRate: { 'General Ward': 800, 'Semi-Private': 1500, 'Private Room': 2500, 'ICU': 4000 }[roomType],
        totalRooms: { 'General Ward': 5, 'Semi-Private': 4, 'Private Room': 3, 'ICU': 4 }[roomType],
        availableRooms: { 'General Ward': 5, 'Semi-Private': 4, 'Private Room': 3, 'ICU': 4 }[roomType],
      }));
  ...
  const handleSaveAll = async () => {
    ...
    setSaveMessage(null);
    try {
      await API.post('/room-settings/bulk', { configs: roomConfigs });
      setSaveMessage({ type: 'success', text: '✅ Room settings saved successfully!' });
      setTimeout(() => setSaveMessage(null), 3000);
    } catch (err) {
      console.error('Save failed', err);
      setSaveMessage({ type: 'error', text: '❌ Failed to save: ' + (err.response?.data?.message || err.message) });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleReset = async () => {
    ...
    setSaving(true);
    try {
      await API.post('/room-settings/bulk', { configs: defaults });
      setRoomConfigs(defaults);
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, fontSize: 13, color: '#1e40af' }}>
          <strong>How it works:</strong> When a patient is admitted, available rooms count decreases automatically. 
          When discharged, it increases back. If available rooms = 0, that room type will be disabled in the admit form.
        </div>
      </div>
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
                ...
                <div style={{
                  ...
                  {isFull ? '❌ No rooms available' : isOverbooked ? '⚠️ Overbooked!' : `✅ ${config.availableRooms} of ${config.totalRooms} available`}
                </div>
              </div>
              ...
              <div style={{ marginTop: 16 }}>
                <div style={{ 
                  ...
                  <div style={{
                    width: `${(config.availableRooms / config.totalRooms) * 100}%`,
                    height: '100%',
                    background: isFull ? '#ef4444' : config.availableRooms < config.totalRooms / 2 ? '#f59e0b' : '#10b981',
                    borderRadius: 3,
      ...
      <div style={{ 
        ...
        <strong>📌 Note:</strong> Changes to "Available Rooms" will immediately affect what receptionists see when admitting patients.
        If a room type shows 0 available, it will be disabled in the admission form.
      </div>
    </div>
  );
}